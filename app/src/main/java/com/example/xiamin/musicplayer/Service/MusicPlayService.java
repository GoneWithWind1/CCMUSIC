...
public class MusicPlayService extends Service implements
    ...
    private static int mPlayingMusicPosition;      //当前播放歌曲的位置
    private PhoneComingReceiver mNoisyReceiver = new PhoneComingReceiver();
    private AudioManager mAudioManager;

      private   int state;   //播放模式


    public static final String UPDATE_ACTION = "com.cqb.action.UPDATE_ACTION";
    ...
    public void ChangeBar(int prossess)
    {
        mediaPlayer.seekTo(prossess);
    }
    ...
    public int next() {
        switch (state) {
            ...
            play(mPlayingMusicPosition + 1);
                  break;
            case 1:                          //随机播放
                mPlayingMusicPosition=getRandomIndex(getMusicList().size());
                play(mPlayingMusicPosition);
                break;
            case 2:                             //单曲循环
                play(mPlayingMusicPosition);
                break;
    ...
    public static long getPlayDuratin() {return mediaPlayer.getDuration();}
